...
public class DBFWriter extends DBFBase implements java.io.Closeable {
  ...
  /**
   * Creates a DBFWriter which write data to the given OutputStream. Uses default charset iso-8859-1
   *
   * @param out stream to write the data to.
   */
  public DBFWriter(OutputStream out) {
    this(out, defaultCharset);
  }
  /**
   ...
   * @exception DBFException if the passed in file does exist but not a valid DBF file, or if an IO
   *     error occurs.
   */
   ...
   *     error occurs.
   */
  public DBFWriter(File dbfFile, boolean overwrite) {
    this(dbfFile, overwrite, null);
  }
   ...
   *     error occurs.
   */
  public DBFWriter(File dbfFile, boolean overwrite, Charset charset) {
    super();
    ...
    try {
      ...
      if (dbfFile.length() == 0) {
        if (charset != null) {
          ...
          this.header.setUsedCharset(defaultCharset);
        }
        return;
      }
  ...
  /**
   * Sets fields definition. To keep maximum compatibility a maximum of 255 columns should be used.
      ...
      if (!field.getDBFType().isWriteSupported()) {
        throw new DBFException(
            "Field "
                ...
                + " is of type "
                + field.getDBFType()
                + " that is not supported for writting");
      }
    }
  ...
  public void addRecord(Object[] values) {
    ...
    for (int i = 0; i < this.header.fieldArray.length; i++) {
      ...
      switch (this.header.fieldArray[i].getDBFType()) {
        case CHARACTER:
        ...
        default:
          throw new DBFException(
              "Unsupported writting of field type "
                  + i
                  + " "
                  + this.header.fieldArray[i].getDBFType());
      }
    }
  ...
  private void writeRecord(DataOutput dataOutput, Object[] objectArray) throws IOException {
    ...
    for (int j = 0; j < this.header.fieldArray.length; j++) {
      ...
      switch (this.header.fieldArray[j].getDBFType()) {
        case CHARACTER:
          String strValue = "";
        ...
        default:
          throw new DBFException("Unknown field type " + this.header.fieldArray[j].getDBFType());
      }
    }
  }
  ...
  /**
   * Open a DBF file in sync mode and set its fields. Use UTF-8 encoding.
   *
   * @param fileName canonical file name.
  ...
  /**
   * Open a DBF file in sync mode and set its fields.
   *
   * @param fileName canonical file name.
   * @param fields fields definitions.
   * @param charset the charset used to encode field name and field contents.
