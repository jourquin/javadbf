 ...
 *
 * <p>DBFField objects are either created and added to a DBFWriter object or obtained from DBFReader
 * object through {@link DBFReader#getField(int)} query.
 */
public class DBFField {
   ...
   */
  @Deprecated public static final byte FIELD_TYPE_C = (byte) 'C';
   ...
   */
  @Deprecated public static final byte FIELD_TYPE_L = (byte) 'L';
   ...
   */
  @Deprecated public static final byte FIELD_TYPE_N = (byte) 'N';
   ...
   */
  @Deprecated public static final byte FIELD_TYPE_F = (byte) 'F';
   ...
   */
  @Deprecated public static final byte FIELD_TYPE_D = (byte) 'D';
   ...
   */
  @Deprecated public static final byte FIELD_TYPE_M = (byte) 'M';
   ...
   *
   * <p>The data in the DataInputStream object is supposed to be organised correctly and the stream
   * "pointer" is supposed to be positioned properly.
   *
   * @param in DataInputStream
   * @param charset charset to use
   * @param useFieldFlags If the file can store field flags (setting this to false ignore any data
   *     in byes 18-19)
   * @param supportExtendedCharacterFields True if 2-byte (extended) length character fields should
   *     be supported, see adjustLengthForLongCharSupport()
  ...
  protected static DBFField createField(
      DataInput in, Charset charset, boolean useFieldFlags, boolean supportExtendedCharacterFields)
      throws IOException {
  ...
  protected static DBFField createFieldDB7(
  ...
  private static void adjustLengthForLongCharSupport(DBFField field) {
    // if field type is char or varchar, then read length and decimalCount as one number to allow
    // char fields to be
    // longer than 256 bytes.
    // This is the way Clipper and FoxPro do it, and there is really no downside
    // since for char fields decimal count should be zero for other versions that do not support
  ...
  /**
   * Returns the decimal part. This is applicable only if the field type if of numeric in nature.
   *
   * <p>If the field is specified to hold integral values the value returned by this method will be
   * zero.
   *
  ...
  /**
   * Set Length of the field. This method should be called before calling setDecimalCount().
   *
    ...
    if (length > this.type.getMaxSize() || length < this.type.getMinSize()) {
      throw new UnsupportedOperationException(
          "Length for "
              + this.type
              + " must be between "
              + this.type.getMinSize()
              + " and "
              + this.type.getMaxSize());
    }
    this.length = length;
  }

  /**
   * Gets the type for this field. Renamed from "getType" in original code, for Nodus API
   * compatibility.
   *
  ...
  /**
   * Set the type for this field. Renamed from "setType" in original code, for Nodus API
   * compatibility.
   *
   * @param type The type for this field
   * @throws IllegalArgumentException if type is not write supported
   */
  public void setDBFType(DBFDataType type) {
  ...
  /**
   * Sets the decimal place size of the field. Before calling this method the size of the field
  ...
  public void setDecimalCount(int size) {
    ...
    if (this.type != DBFDataType.NUMERIC && this.type != DBFDataType.FLOATING_POINT) {
      throw new UnsupportedOperationException(
          "Cannot set decimal count on this field:" + this.type);
    }
   ...
   *     C, L, N, F, D, M
   * @deprecated This method is deprecated and is replaced by {@link #setDBFType(DBFDataType)}
   */
  @Deprecated
  public void setDataType(byte type) {
    setDBFType(DBFDataType.fromCode(type));
  }
   ...
   * @deprecated This method is deprecated and is replaced by {@link #getDBFType()}
   */
  @Deprecated
   ...
   *
   * @param name Field name
   * @param type Field type (char format)
   * @param length Field length
   * @param decimalCount number of decimal places to use
   */
  public DBFField(String name, char type, int length, int decimalCount) {
    super();
    setName(name);
    setDBFType(getJavaDBFType(type));
    setFieldLength(length);
    if (decimalCount != 0) {
      setDecimalCount(decimalCount);
    }
  ...
  /**
   * Get the DBFDataType for a give char code.
   *
   * @param charType char code
   * @return DBFDataType
   */
  private DBFDataType getJavaDBFType(char charType) {
    switch (charType) {
      case 'C':
      ...
      case '@':
        return DBFDataType.TIMESTAMP_DBASE7;
      default:
        return DBFDataType.UNKNOWN;
    }
  }

  /**
   * Get the char code of the field type.
   *
   * @return char code.
   */
  public char getType() {
