 ...
 * Sources: https://github.com/ladislav-zezula/StormLib/blob/master/src/pklib/explode.c
 * https://github.com/toshok/scsharp/blob/master/SCSharp/SCSharp.Mpq/PKLibDecompress.cs
 *
 * <p>https://code.google.com/p/arx-fatalis-fixed/source/browse/trunk/Sources/HERMES/explode.c?r=23
...
public class DBFExploder {
  private static final int PK_LITERAL_SIZE_FIXED =
      0; // Use fixed size literal bytes, used for binary data
  private static final int PK_LITERAL_SIZE_VARIABLE =
      1; // Use variable size literal bytes, used for text
  ...
  public static int pkexplode(byte[] pInBuffer, DBFStorage out, int outSize) throws IOException {
    ...
    while (pOutPos < outSize) {
      ...
      if ((nBitBuffer & 1) != 0) {
        ...
        for (i = 0; i <= 0x0F; i++) {
          if (TRUNCATE_VALUE(nBitBuffer, LenBits[i] & 0xFF) == (LenCode[i] & 0xFF)) break;
        }
        ...
        nCopyLen =
            (int)
                ((LenBase[i] & 0xFFFF)
                    + TRUNCATE_VALUE(
                        nBitBuffer,
                        ExLenBits[i] & 0xFF)); // Length of data to copy from the dictionary
        ...
        for (i = 0; i <= 0x3F; i++) {
          if (TRUNCATE_VALUE(nBitBuffer, OffsBits[i] & 0xFF) == (OffsCode[i] & 0xFF)) break;
        }
        ...
        if (nCopyLen == 2) {
          ...
          pCopyOffs =
              (int)
                  (pDictPos
                      - 1
                      - ((i << nDictSizeByte) + TRUNCATE_VALUE(nBitBuffer, nDictSizeByte)));
      ...
      // First bit is 0; literal byte
        ...
        // Variable size literal byte
          ...
          for (i = 0; i <= 0xFF; i++) {
            if (TRUNCATE_VALUE(nBitBuffer, ChBits[i] & 0xFF) == (ChCode[i] & 0xFFFF)) break;
          }
  ...
  static class OutputStreamStorage implements DBFStorage {
    ...
    private int addByteAndWrite(OutputStream out, byte[] buffer, int currentFillSize, byte b)
        throws IOException {
      buffer[currentFillSize++] = b;
      if (currentFillSize >= bufferLength) {
        out.write(buffer);
        return 0;
      } else return currentFillSize;
    }
  }
